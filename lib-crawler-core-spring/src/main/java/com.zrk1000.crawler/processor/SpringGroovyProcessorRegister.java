 ...
 * 重写GroovyProcessorRegister的doInject()方法 ，添加对spring的@Autowired注解的支持
 * User: zrk-PC
 * Date: 2017/9/11
 * Time: 18:28
 */
public class SpringGroovyProcessorRegister extends GroovyProcessorRegister {

    private static Logger logger = LoggerFactory.getLogger(SpringGroovyProcessorRegister.class);

    public SpringGroovyProcessorRegister(File directory, ServiceFactory serviceFactory) {
        super(directory, serviceFactory);
    }

    public SpringGroovyProcessorRegister(File directory, IOFileFilter fileFilter, ServiceFactory serviceFactory, int delay, long period) {
